...
def main(data_src):
    ...
    val_df = pd.DataFrame({'review': [os.path.basename(path) for path in val_texts], 'predicted_sentiment': val_preds.astype(int)})
    val_df.to_csv('val_predictions.csv', index=False)

    test_df = pd.DataFrame({'review': [os.path.basename(path) for path in test_texts], 'predicted_sentiment': test_preds.astype(int)})
